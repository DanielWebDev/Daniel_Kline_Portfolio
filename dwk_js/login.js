...
(function() {
	...
	function validateForm(e) {
		...
		if ( (name.length > 0) && (email.length > 0) && (comment.length > 0) ) {
			...
			var data = 'name=' + encodeURIComponent(name) + '&email=' + encodeURIComponent(email) + 'comment=' + encodeURIComponent(comment);
			ajax.send(data);    
			    ...
			    var message = '';
			    if (name.length == 0) {
			        message += '<li class="error">You forgot to enter your name!</li>'
			    }
			    if (email.length == 0) {
			        message += '<li class="error">You forgot to enter your email!</li>'
			    }
			    if (comment.length == 0) {
			        message += '<li class="error">You forgot to enter a comment!</li>'
			    }
			    message += '</ul></p>';
